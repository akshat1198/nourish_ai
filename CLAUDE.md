...
- **Source `servings` is unreliable, and that is the source's opinion, not an import bug.** Stored servings match `archana_dataset.csv` exactly on all 6,728 rows, so there is nothing to re-import: Archana's genuinely claims 2 servings for 750 g of chicken, and TheMealDB hardcodes 4 for all ~700 of its rows. On the LLM backfill the model read a different count on 157 of 389 recipes. Changing it means overriding the recipe author, which is a product decision rather than a data fix.
- **Generic source names alias to specific canonicals** (`chicken` → `chicken breast`, `cheese` → `paneer`), so a bone-in bird is priced at breast-meat macros. Measured before acting on it: 244 recipes (3.2%), of which 23 already carry LLM estimates and 70 are derived with >40 g protein — and those read plausibly (Kerala Chicken Curry 342 kcal / 41.5 g), because the skew is ~1.5x on one macro, not an order of magnitude. Splitting the vocabulary would need re-reading source data (`backfill_ingredients.py --relabel` overwrote the display names, so the raw term is gone from the DB) and would change **pantry matching** as well as nutrition. Deliberately not done: the cost and blast radius exceed a 1.5x skew on 3% of rows. Re-measure before revisiting.
